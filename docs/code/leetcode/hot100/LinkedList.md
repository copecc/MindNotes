...
围绕这类局部反转，常见追问会继续扩展到只反转前 `N` 个节点、反转区间 `[m,n]`，以及每 `k` 个节点一组反转。它们的共同点都是先切清楚边界，再只在局部区间内做指针重连；一旦边界判断出错，后续链表就容易断开或重复连接。
...
???+ solution "206. 反转链表"
    ...
    === "递归"
        ...
        ```cpp
        --8<-- "code/leetcode/hot100/L206_2.cpp"
        ```

作为最典型的区间版本，[92. 反转链表 II](https://leetcode.cn/problems/reverse-linked-list-ii/description/){target=_blank} 要求只翻转中间一段。它和整表反转的区别不在“怎么反转”，而在“如何把待反转区间从整条链表中切出来，再把前后两端重新接回去”。

从模板关系看，这一组题可以按边界控制来统一：`206` 是整表反转，边界覆盖整条链表；`92` 是区间反转，边界固定在 `[left, right]`；`25` 则是把区间反转重复应用到长度为 `k` 的连续分组上。三者复用的都是局部指针重连，真正变化的是区间如何定位、何时停止，以及反转后怎样把前驱与后继重新接回。

??? note "92. 反转链表 II（衍生）"

    这题常见两种写法：递归写法把问题化成“反转前 `N` 个节点”并逐层缩小左边界；迭代写法则先找到区间前驱，再在区间内部反复做头插重连。

    === "递归"

        ```cpp
        --8<-- "code/leetcode/hot100/L92_1.cpp"
        ```

    === "迭代穿针"

        ```cpp
        --8<-- "code/leetcode/hot100/L92_2.cpp"
        ```
...
如果把两路归并继续推广，就得到合并 `k` 条有序链表。常见扩展有两类：一类是用小根堆维护当前 `k` 个头节点中的最小值，另一类是按分治方式不断做两两归并，本质上仍然复用了这道题的基本操作。
...
???+ solution "21. 合并两个有序链表"
    ...
    === "递归"
        ...
        ```cpp
        --8<-- "code/leetcode/hot100/L21_2.cpp"
        ```
...
???+ solution "2. 两数相加"
    ...
    === "递归"
        ...
        ```cpp
        --8<-- "code/leetcode/hot100/L2_2.cpp"
        ```
...
从递归视角看，这题是“先交换前两个节点，再把后续子问题接回去”；从迭代视角看，则是反复执行固定的四指针重连。前者更接近局部结构归纳，后者更强调显式维护前驱与边界。
...
???+ solution "24. 两两交换链表中的节点"
    ...
    === "递归"
        ...
        ```cpp
        --8<-- "code/leetcode/hot100/L24_2.cpp"
        ```
...
???+ solution "25. K 个一组翻转链表"
    ...
    === "递归"
        ...
        ```cpp
        --8<-- "code/leetcode/hot100/L25_2.cpp"
        ```
...
这题还有一种常见优化是穿插复制：先把新节点插到原节点后面，再利用相邻关系补齐 `random` 指针，最后把新旧链表拆开。它省掉了显式哈希表，但要求对三轮局部重连的顺序控制更严格。
...
???+ solution "138. 随机链表的复制"
    ...
    === "穿插复制"
        ...
        ```cpp
        --8<-- "code/leetcode/hot100/L138_2.cpp"
        ```
...
另一类常见写法是分治归并：不断把链表集合拆成两半，分别合并后再归并结果。这样把多路问题重新降回两路归并，时间复杂度同样可以控制在 $O(N \log k)$。
...
???+ solution "23. 合并 K 个升序链表"
    ...
    === "分治"
        ...
        ```cpp
        --8<-- "code/leetcode/hot100/L23_2.cpp"
        ```
