...
!!! problem "[136. 只出现一次的数字](https://leetcode.cn/problems/single-number/description/?envType=study-plan-v2&envId=top-100-liked){target=_blank}"
    ...
    给你一个非空整数数组 `nums`，其中除了某个元素只出现一次以外，其余每个元素都恰好出现两次。请找出那个只出现一次的元素并返回。

    题目要求算法的时间复杂度为线性级别，并且只能使用常量额外空间。输入数组长度至少为 `1`，答案一定存在。
...
!!! problem "[260. 只出现一次的数字 III](https://leetcode.cn/problems/single-number-iii/description/){target=_blank}"
    ...
    给你一个整数数组 `nums`，其中恰好有两个元素只出现一次，其余所有元素都恰好出现两次。请找出这两个只出现一次的元素并返回。

    题目要求算法必须是线性时间复杂度，并且只能使用常量额外空间。返回顺序不作要求，只要两个目标值都被找出即可。
...
!!! problem "[137. 只出现一次的数字 II](https://leetcode.cn/problems/single-number-ii/description/){target=_blank}"
    ...
    给你一个整数数组 `nums`，其中只有一个元素只出现一次，其余每个元素都恰好出现三次。请找出并返回那个只出现一次的元素。

    题目要求算法的时间复杂度为线性级别，且不能使用额外空间。输入数组中的重复元素次数固定为 `3`，答案一定存在。
...
!!! problem "[169. 多数元素](https://leetcode.cn/problems/majority-element/description/?envType=study-plan-v2&envId=top-100-liked){target=_blank}"
    ...
    给定一个大小为 `n` 的非空数组 `nums`，请返回其中的多数元素。多数元素是指在数组中出现次数严格大于 `⌊n / 2⌋` 的元素。

    题目保证多数元素一定存在，因此返回值也一定唯一。输入只需要一次遍历即可完成判断。
...
!!! problem "[75. 颜色分类](https://leetcode.cn/problems/sort-colors/description/?envType=study-plan-v2&envId=top-100-liked){target=_blank}"
    ...
    给定一个包含 `n` 个元素的数组 `nums`，其中只包含整数 `0`、`1` 和 `2`，它们分别表示红色、白色和蓝色。请原地对数组进行排序，使相同颜色的元素相邻，并按红色、白色、蓝色的顺序排列。

    题目要求不能使用库内置排序函数，只能在原数组上完成重排。返回值不需要额外构造，结果直接体现在 `nums` 上。
...
!!! problem "[31. 下一个排列](https://leetcode.cn/problems/next-permutation/description/?envType=study-plan-v2&envId=top-100-liked){target=_blank}"
    ...
    给定一个整数数组 `nums`，它表示一个排列。请将其原地修改为字典序中的下一个更大排列。

    如果不存在下一个更大的排列，就把数组重排成字典序最小的排列。题目要求只使用额外常数空间，并且直接在原数组上完成修改。
...
!!! problem "[287. 寻找重复数](https://leetcode.cn/problems/find-the-duplicate-number/description/?envType=study-plan-v2&envId=top-100-liked){target=_blank}"
    ...
    给定一个包含 `n + 1` 个整数的数组 `nums`，其中每个整数都在 `[1, n]` 范围内。由于数组长度比值域多 `1`，因此至少存在一个重复的整数。

    题目保证只有一个整数会重复出现，但它可能出现两次或更多次。请在不能修改原数组、且只使用 `O(1)` 额外空间的前提下，返回这个重复的数。
