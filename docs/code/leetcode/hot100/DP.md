...
这题的主写法通常是完全背包 DP，但也可以从图搜索角度理解。

若按完全背包建模，设 `dp[j]` 表示凑出金额 `j` 的最少硬币数。对每个硬币 `coin`，从小到大更新：

$$
dp[j] = \min(dp[j],\ dp[j - coin] + 1)
$$

这样状态直接表示“达到某个金额所需的最少步数”。

另一种常见变体是把金额看成图上的节点：从金额 `x` 可以走到 `x + coin`，每走一步就代表再使用一枚硬币。因为每条边的代价都相同，所以从 `0` 出发做 BFS，第一次到达 `amount` 时，对应的层数就是最少硬币数。

DP 更贴近背包模板；BFS 更强调“最少步数”这一结构，两者本质上都在解同一个最短路问题。
...
???+ solution "322. 零钱兑换"
    ...
    === "BFS"
        ...
        ```cpp
        --8<-- "code/leetcode/hot100/L322_2.cpp"
        ```
...
这题既可以写成前缀 DP，也可以写成 DFS 加记忆化搜索。

若按前缀 DP，设 `dp[i]` 表示前 `i` 个字符能否被成功拆分。只要存在一个切分点 `j`，满足 `dp[j] = true` 且 `s[j:i]` 在字典中，就可以令 `dp[i] = true`。这种写法把问题看成“前缀是否可达”。

若按记忆化搜索，则从某个起点 `start` 出发，递归尝试所有可能的下一段单词；一旦某个后缀能够成功拆分，就立即返回成功。为了避免重复搜索，需要把每个起点是否可拆分缓存下来。

两种写法本质上都在枚举切分点，只是 DP 按前缀长度自底向上推进，记忆化搜索按起点位置自顶向下展开。
...
???+ solution "139. 单词拆分"
    ...
    === "记忆化搜索"
        ...
        ```cpp
        --8<-- "code/leetcode/hot100/L139_2.cpp"
        ```
...
这题最经典的两种写法分别对应两套状态表示。

一种是朴素动态规划：令 `dp[i]` 表示以 `nums[i]` 结尾的最长递增子序列长度，那么只要枚举所有 `j < i` 且 `nums[j] < nums[i]` 的位置，就可以转移到 `dp[i]`。这种写法直接体现“当前位置接在谁后面”。

另一种是贪心加二分。维护数组 `tails`，其中 `tails[k]` 表示长度为 `k + 1` 的递增子序列所能取得的最小结尾值。遍历到新数时：若它比所有结尾都大，就扩展长度；否则替换第一个大于等于它的位置，把同长度子序列的结尾尽量压小。

前者容易从状态定义直接推出，时间复杂度是 $O(n^2)$；后者不显式记录每个位置的答案，但把复杂度优化到 $O(n \log n)$。
...
???+ solution "300. 最长递增子序列"
    ...
    === "DP"
        ...
        ```cpp
        --8<-- "code/leetcode/hot100/L300_2.cpp"
        ```
