...
2. 区间查询  
      ...
      - 前缀和：
        $\begin{aligned}
        \sum_{i=1}^r A[i]
        &= \sum_{i=1}^r \sum_{j=1}^i D[j]
        = rD[1] + (r-1)D[2] + \ldots + (r-(r-1))D[r] \\
        &= r\sum_{i=1}^r D[i] - \sum_{i=1}^r (i-1)D[i]
        \end{aligned}$  
      - 令 $F[i]=(i-1)D[i]$，则
        $\sum_{i=l}^r A[i]=r\sum_{i=1}^r D[i]-\sum_{i=1}^r F[i]-((l-1)\sum_{i=1}^{l-1} D[i]-\sum_{i=1}^{l-1} F[i])$
