...
`matrix_power` 的整体控制流是宿主端快速幂，但它决定计算成本的核心仍然是矩阵乘 kernel。因此在模式归类上，它更适合视为 GEMM 家族的一个朴素变体，而不是单独拆成新的 kernel 类型。

若矩阵幂为 $A^p$，快速幂会把指数拆成若干次平方和乘法；每一步的核心计算仍是：

$$
C_{i,j} = \sum_{k=0}^{N-1} A_{i,k}B_{k,j}
$$

它所使用的 launch 方式仍然是标准二维映射：

$$
row = blockIdx.y \times blockDim.y + threadIdx.y, \quad col = blockIdx.x \times blockDim.x + threadIdx.x
$$

每个线程负责一个输出元素，对应最基础的 `one-thread-per-output-element` 矩阵乘结构。

???+ note "matrix_power 实现"

    === "Basic"

        ```cpp title="matrix_power_basic"
        --8<-- "learning/cuda/kernel/matrix_power_basic.cu"
        ```

这一实现没有使用 shared memory tile、寄存器分块或向量化访存，因此更适合作为 GEMM 优化路线中的起点参考。
