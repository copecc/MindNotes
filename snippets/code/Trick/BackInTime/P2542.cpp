...
struct segment_tree {
  ...
  explicit segment_tree(int64_t n) : sum(n * 4), tag_set(n * 4), tag_set_val(n * 4) {}

  void push_up(int64_t i) { sum[i] = sum[2 * i] + sum[2 * i + 1]; }

  // 构建线段树
  void build(int64_t i, int64_t left, int64_t right) {
...
int main() {
  ...
  auto path_set = [&](int64_t u, int64_t v, int64_t val) {
    while (top[u] != top[v]) {
      if (depth[top[u]] < depth[top[v]]) { swap(u, v); }
  ...
  for (int64_t ans : answer) { cout << ans << "\n"; }
